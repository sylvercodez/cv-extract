<?php
    ...
    class Payments extends CI_Controller {
        ...
        public function index(){
            $payment = $this->payment->select();
            $data['payment'] = $payment;
            $data['action'] = 'view';
            $data['header_active'] = "payments";
            ...
            $this->load->newAdmin('payments',$data);
        }
        public function add(){
        ...
        public function select(){
            $column = array(
                ...
                1=>array('column'=>'transaction_ref','prefix'=>''),
                2=>array('column'=>'user_id','prefix'=>''),
                3=>array('column'=>'status','prefix'=>''),
                4=>array('column'=>'cv_id','prefix'=>''),
            ...
            $where = $this->payment->filter($_REQUEST,$column);
            $orderby = $this->payment->order($_REQUEST,$column);
            
            $total_records = $this->payment->count($status);
            $records = $this->payment->select($status,$where,$orderby,$start,$length);
            $filtered_records = $this->payment->count($status,$where);
            
            $record = array();
            foreach($records as $key=>$val){
				$user = $this->User->select('', "id = $val->user_id")[0];
                $array = array();
                $array[] = strval($this->input->get('start')+$key+1);
                $array[] = $val->transaction_ref;
                // $array[] = '<img src="'.asset_url('users/'.$val->image).'" width="100px;" />';
                $array[] = $user->email;
                $array[] = $val->status == 1 ? "Success" : "Pending";
                $array[] = $val->cv_id;
                $array[] = $val->price;
                // $array[] = $val->country;
